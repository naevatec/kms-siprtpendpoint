...
static guint
kms_sip_rtp_connection_get_rtcp_port (KmsRtpBaseConnection * base_conn)
{
  KmsSipRtpConnection *self = KMS_SIP_RTP_CONNECTION (base_conn);
...
static void
kms_sip_rtp_connection_set_remote_info (KmsRtpBaseConnection * base_conn,
    const gchar * host, gint rtp_port, gint rtcp_port)
{
  KmsSipRtpConnection *self = KMS_SIP_RTP_CONNECTION (base_conn);
  KmsSipRtpConnectionPrivate *priv = self->priv;
...
static void
kms_sip_rtp_connection_add (KmsIRtpConnection * base_rtp_conn, GstBin * bin,
    gboolean active)
{
...
static void
kms_sip_rtp_connection_src_sync_state_with_parent (KmsIRtpConnection *
    base_rtp_conn)
{
  KmsSipRtpConnection *self = KMS_SIP_RTP_CONNECTION (base_rtp_conn);
  KmsSipRtpConnectionPrivate *priv = self->priv;
...
static void
kms_sip_rtp_connection_sink_sync_state_with_parent (KmsIRtpConnection *
    base_rtp_conn)
{
  KmsSipRtpConnection *self = KMS_SIP_RTP_CONNECTION (base_rtp_conn);
  KmsSipRtpConnectionPrivate *priv = self->priv;
...
static GstPad *
kms_sip_rtp_connection_request_rtp_src (KmsIRtpConnection * base_rtp_conn)
{
  KmsSipRtpConnection *self = KMS_SIP_RTP_CONNECTION (base_rtp_conn);
...
static GstPad *
kms_sip_rtp_connection_request_rtcp_sink (KmsIRtpConnection * base_rtp_conn)
{
  KmsSipRtpConnection *self = KMS_SIP_RTP_CONNECTION (base_rtp_conn);
...
static GstPad *
kms_sip_rtp_connection_request_rtcp_src (KmsIRtpConnection * base_rtp_conn)
{
  KmsSipRtpConnection *self = KMS_SIP_RTP_CONNECTION (base_rtp_conn);
...
static void
kms_sip_rtp_connection_enable_latency_stats (KmsRtpBaseConnection * base)
{
...
void
kms_sip_rtp_transport_disable_latency_notification (KmsRtpBaseConnection * base)
{
  KmsSipRtpConnection *self = KMS_SIP_RTP_CONNECTION (base);
...
kms_sip_rtp_connection_finalize (GObject * object)
{
  ...
  kms_sip_rtp_transport_disable_latency_notification (KMS_RTP_BASE_CONNECTION
      (self));
...
kms_sip_rtp_connection_class_init (KmsSipRtpConnectionClass * klass)
{
  ...
  base_conn_class->get_rtp_port = kms_sip_rtp_connection_get_rtp_port;
  base_conn_class->get_rtcp_port = kms_sip_rtp_connection_get_rtcp_port;
  base_conn_class->set_remote_info = kms_sip_rtp_connection_set_remote_info;
...
kms_sip_rtp_connection_interface_init (KmsIRtpConnectionInterface * iface)
{
  ...
  iface->src_sync_state_with_parent =
      kms_sip_rtp_connection_src_sync_state_with_parent;
