...
#include <gst/gst.h>
#include <kurento/rtpendpoint/kmsrtpsession.h>
...
struct _KmsSipRtpSessionClass
{
  ...
  void (*store_rtp_filtering_info) (KmsSipRtpSession *ses, KmsRtpConnection *conn, gulong rtp_probe, gulong rtcp_probe, gulong rtp_sink_signal, gulong rtcp_sink_signal);

};

GType kms_sip_rtp_session_get_type (void);

KmsSipRtpSession * kms_sip_rtp_session_new (KmsBaseSdpEndpoint * ep, guint id, KmsIRtpSessionManager * manager, gboolean use_ipv6, gint audio_dscp_value, gint video_dscp_value);
