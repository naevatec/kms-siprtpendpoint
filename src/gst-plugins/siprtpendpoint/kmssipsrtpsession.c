...
G_DEFINE_TYPE (KmsSipSrtpSession, kms_sip_srtp_session, KMS_TYPE_BASE_RTP_SESSION);
    ...
    KmsIRtpSessionManager * manager, gboolean use_ipv6)
{
  ...
  KMS_SIP_SRTP_SESSION_CLASS (G_OBJECT_GET_CLASS (self))->post_constructor
...
kms_sip_srtp_session_create_connection (KmsBaseRtpSession * base_rtp_sess,
    ...
    guint16 max_port)
{
  KmsSipSrtpConnection *conn = kms_sip_srtp_connection_new (min_port, max_port,
      KMS_SIP_SRTP_SESSION (base_rtp_sess)->use_ipv6);
...
static void
kms_sip_srtp_session_post_constructor (KmsSipSrtpSession * self,
...
kms_sip_srtp_session_class_init (KmsSipSrtpSessionClass * klass)
{
  ...
  klass->post_constructor = kms_sip_srtp_session_post_constructor;
