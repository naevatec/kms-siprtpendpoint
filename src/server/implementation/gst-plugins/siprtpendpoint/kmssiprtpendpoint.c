    ...
    gint id, KmsSdpSession ** sess)
{
  ...
  KMS_BASE_SDP_ENDPOINT_CLASS
      (kms_sip_rtp_endpoint_parent_class)->create_session_internal (base_sdp, id,
      sess);
}
...
kms_sip_rtp_endpoint_get_media_handler (KmsSipRtpEndpoint * self, const gchar * media)
{
  ...
  if (self->priv->use_sdes) {
    handler = kms_sip_rtp_endpoint_provide_sdes_handler (self, media);
  } else {
    ...
    KmsSdpSession * sess, KmsSdpMediaHandler * handler, GstSDPMedia * media)
{
  ...
  ret = KMS_BASE_SDP_ENDPOINT_CLASS
      (kms_sip_rtp_endpoint_parent_class)->configure_media (base_sdp_endpoint, sess,
      handler, media);
  if (ret == FALSE) {
    media->port = 0;
  ...
  conn = kms_sip_rtp_endpoint_get_connection (KMS_SIP_RTP_ENDPOINT (base_sdp_endpoint),
      sess, handler, media);
  ...
  if (self->priv->use_sdes) {
    kms_sip_rtp_endpoint_configure_connection_keys (self, conn,
    ...
    KmsSdpSession *sess, gboolean offerer)
{
  ...
  for (guint i = 0; i < len; i++) {
    ...
    if (comedia_enabled) {
      ...
      kms_sip_rtp_endpoint_comedia_manager_create(self, media, conn);
    }
    else {
      const gchar *media_str = gst_sdp_media_get_media (media);
